...
export const loginDeviceByIp = async (
  ...
  const response = await axios
    .post(`http://${deviceIp}/app/handshake1`, localSeed, {
  ...
  const sessionCookie = setCookieHeader.substring(
    0,
  ...
  await axios
    .post(`http://${deviceIp}/app/handshake2`, payload, {
      responseType: "arraybuffer",
      headers: {
...
const deriveKey = (localSeed: Buffer, remoteSeed: Buffer, userHash: Buffer) =>
  sha256(Buffer.concat([encode("lsk"), localSeed, remoteSeed, userHash])).slice(
    0,
    16,
  );
...
const deriveSig = (localSeed: Buffer, remoteSeed: Buffer, userHash: Buffer) =>
  sha256(Buffer.concat([encode("ldk"), localSeed, remoteSeed, userHash])).slice(
    0,
    28,
  );
