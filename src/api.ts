...
import { TapoDevice as CloudTapoDevice } from "./tapo-device";
...
export const cloudLogin = async (
  email: string = process.env.TAPO_USERNAME || undefined,
  password: string = process.env.TAPO_PASSWORD || undefined,
) => {
  const loginRequest = {
    method: "login",
    params: {
  ...
  const listDevices = async (): Promise<Array<TapoDevice>> => {
    ...
    return Promise.all(
      response.data.result.deviceList.map(async (deviceInfo) =>
        augmentTapoDevice(deviceInfo),
      ),
    );
  };

  const listDevicesByType = async (
    deviceType: string,
  ): Promise<Array<TapoDevice>> => {
    const devices = await listDevices();
    return devices.filter((d) => d.deviceType === deviceType);
  };

  const tapoCareCloudVideos = async (
    deviceId: string,
    order: string = "desc",
    page: number = 0,
    pageSize: number = 20,
  ...
  const getTapoDevice = (device: TapoDevice) => {
    const send = async (request: any) => {
      ...
      checkError(response.data);
      return JSON.parse(response.data.result.responseData);
    };
    return {
      turnOn: async () =>
        await send({
...
export const loginDevice = async (
  ...
  if (!localIp) {
    throw new Error(
      `Local IP of device with Mac address ${device.deviceMac} not found.`,
    );
  }
...
export const loginDeviceByIp = async (
  email: string = process.env.TAPO_USERNAME || undefined,
  password: string = process.env.TAPO_PASSWORD || undefined,
  deviceIp: string,
) => {
...
const checkTapoCareError = (responseData: any) => {
  ...
  if (errorCode) {
    throw new Error(
      `Unrecognised Error Code: ${errorCode} (${responseData["message"]})`,
    );
  }
};
